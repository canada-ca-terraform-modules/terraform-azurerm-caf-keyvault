...
| `variables.tf` | `env`, `location` (defaults to `canadacentral`), `tags`, `pr_number` (defaults to `"manual"`), and `akv_config` (typed `any`, passed straight through to the module). |
...
terraform apply -var-file=config/key_vault.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/key_vault.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/key_vault.tfvars -var="pr_number=<pr-number>"
```
...
group. The module's own Key Vault naming derives from
`sha1(resource_group.id)`, so a distinct per-PR resource group also yields a
distinct per-PR Key Vault name with no further changes needed.
